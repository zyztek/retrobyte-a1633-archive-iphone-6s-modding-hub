...
export function TestCenterPage() {
  ...
  return (
    <RetroLayout>
      <div className={cn("space-y-10 transition-opacity duration-500", isRunning && "opacity-80 pointer-events-none")}>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <button
            ...
            disabled={isRunning}
            className="retro-button border-neon-pink text-neon-pink shadow-[4px_4px_0px_rgba(210,9,250,1)] hover:shadow-none flex items-center gap-2 disabled:opacity-50"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-1 overflow-hidden">
                    {Array.from({ length: 12 }).map((_, i) => (
                      <div key={i} className="h-4 w-1 bg-emerald-500/40 shrink-0" />
                    ))}
                  </div>
                </div>
              </RetroCard>
            ...
            <Tabs defaultValue="workflow" className="w-full">
              <TabsList className="bg-retro-black border-2 border-neon-green/30 rounded-none w-full flex justify-start p-1 gap-1 h-auto flex-wrap">
                <TabsTrigger value="workflow" className="rounded-none border-2 border-transparent data-[state=active]:border-neon-green data-[state=active]:bg-neon-green/10 text-[10px] font-bold uppercase py-2 flex-1 min-w-[120px]">
                  <History className="size-3 mr-2" /> Workflow_History
                </TabsTrigger>
                <TabsTrigger value="config" className="rounded-none border-2 border-transparent data-[state=active]:border-neon-green data-[state=active]:bg-neon-green/10 text-[10px] font-bold uppercase py-2 flex-1 min-w-[120px]">
                ...
                <RetroCard title="GITHUB_ACTIONS_LOG">
                  <div className="space-y-3">
                    {WORKFLOW_HISTORY.map((run) => (
                      <div key={run.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 border-2 border-neon-green/20 bg-neon-green/5 text-[10px] font-bold uppercase gap-2">
                        <div className="flex items-center gap-3">
                          <CheckCircle2 className="size-4 text-neon-green shrink-0" />
                          <span className="truncate">{run.sha}</span>
